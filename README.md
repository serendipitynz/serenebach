...
The admin UI bundles the **Ace** editor (`ajaxorg/ace`, © 2010 Ajax.org B.V., [BSD 3-Clause](web/templates/admin/assets/ace/LICENSE)). The copyright notices and full license texts for Ace and for the direct Go dependencies (chi, goose, goldmark, modernc.org/sqlite, golang.org/x/{crypto,image,text}, yaml.v3) are collected in [THIRD-PARTY-NOTICES.txt](THIRD-PARTY-NOTICES.txt), which is bundled into every release archive. Refresh it from the module cache when `go.mod` direct dependencies change.
