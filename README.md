...
The admin UI bundles the **Ace** editor (`ajaxorg/ace`, © 2010 Ajax.org B.V., [BSD 3-Clause](web/templates/admin/assets/ace/LICENSE)). The copyright notices and full license texts for Ace and for **every Go module compiled into the binary** — direct and indirect dependencies alike, as reported by `go list -deps ./cmd/serenebach` — are collected in [THIRD-PARTY-NOTICES.txt](THIRD-PARTY-NOTICES.txt), which is bundled into every release archive. Regenerate it from the module cache when the binary's dependencies change.
