...
| **画像ファイル** | ❌ 取り込みでは扱いません | 画像は同じサーバに残るので、絶対パス（`/blog/img/…`）参照の画像はそのまま表示。相対パス（`../img/…`）参照は公開後にリンク切れになることがあり修正が必要（[ステップ7](#ステップ7本番のサーバへ公開する)）。別サーバへ移す場合は FTP でコピー |
...
記事の中の画像は、この**ローカル確認の時点では表示されません**。作業用パソコンの中には画像ファイルが無いためで、不具合ではありません。本番サーバ（今と同じサーバ）には画像ファイルが残っているので、公開後は表示されるものが多いですが、**記事が相対パス（`../img/…`）で画像を参照している場合は、新しい記事 URL では参照先がずれてリンク切れになることがあります**（[ステップ7](#ステップ7本番のサーバへ公開する) で対処）。別のサーバへ移す場合は画像フォルダのコピーも必要です。
...
- 記事内の**画像ファイルはすでにサーバ上にある**ので、運び直しは不要です（ただし相対パスで参照している画像はリンク切れになることがあります → 下記）。
...
> 🖼️ **画像ファイル自体は運ばなくて大丈夫**（同じサーバなら `img/` などにそのまま残ります）。ただし、**記事本文の画像リンクの書き方**によって表示できるかが変わります。
> - **絶対パス**（`/blog/img/takao.jpg` や `https://…/img/…`）で書かれた画像 → そのまま表示されます。
> - **相対パス**（`../img/takao.jpg` など。SB2 の静的ページ `log/●●.html` を基準に書かれていることが多い）→ 新しい記事 URL（`/blog/entry/{key}/`）は階層が違うため参照先がずれ、**リンク切れになります**。
> - 対処：公開後に記事を開いて確認し、切れていたら該当記事の画像リンクを **絶対パスに直す**（または画像ライブラリから挿入し直す）。取り込みは本文をそのまま保存するため、パスの自動変換は行いません。
>
> 別のサーバへ移すときは、画像フォルダ自体も FTP でコピーします。
...
- [ ] 記事内の画像が表示される（相対パス参照の画像はリンク切れになりうる。切れていたら絶対パスに直す）
...
| 画像が表示されない | 画像リンクが相対パス（`../img/…`）だと、新しい記事 URL では参照先がずれてリンク切れになります。該当記事の画像リンクを**絶対パス**（`/blog/img/…`）に直すか、画像ライブラリから挿入し直してください。別のサーバへ移した場合は、[ステップ7](#ステップ7本番のサーバへ公開する) の手順で画像フォルダもコピーします（取り込み自体は画像ファイルを扱いません）。 |
