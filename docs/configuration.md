...
| `SB_CSRF_MULTIPART_MAX_BYTES` | Raw byte cap on how much of a multipart body the CSRF middleware is willing to read pre-authentication while looking for the form-encoded `csrf_token`. Default `1048576` (1 MiB). The JS upload paths in the admin module graph send the token via the `X-CSRF-Token` header instead, so the middleware never touches the body and this cap doesn't apply to them. Raise it only when a `<noscript>` fallback genuinely needs to submit larger multipart forms; the canonical large-upload path is the JS one |
...
| `OAUTH_REDIRECT_URIS` | **Required when `BASE_URL` or `PROXY_LISTEN_ADDR` is non-loopback.** Comma-separated allowlist of `redirect_uri` values. When empty, any URI is accepted (loopback / development only). |
| `AUTH_PIN` | **Required when `BASE_URL` or `PROXY_LISTEN_ADDR` is non-loopback.** If set, the `/authorize` page requires this PIN before issuing a code |
| `BASE_URL` | Public URL of the proxy itself, used in OAuth metadata (default `http://localhost:8080`). When this host or the listen address (see below) is not `localhost` / `127.0.0.1` / `::1`, the proxy refuses to start unless both `AUTH_PIN` and `OAUTH_REDIRECT_URIS` are set (or `PROXY_ALLOW_INSECURE_DEV=1` is given as an explicit override). |
| `PROXY_LISTEN_ADDR` | Listen address for the proxy (default `:8080`). The default binds every interface, which the production guard treats as non-loopback. Use `127.0.0.1:8080` for a safe local-only setup. |
| `TOKEN_TTL` | Access-token lifetime (default `24h`) |
| `PROXY_ALLOW_INSECURE_DEV` | Set to `1` to skip the non-loopback safety check that otherwise demands `AUTH_PIN` and `OAUTH_REDIRECT_URIS`. Triggered by either `BASE_URL` or `PROXY_LISTEN_ADDR`. Development / closed networks only |
