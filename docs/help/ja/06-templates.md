...
| 記事関連 | `entry_canonical` | **個別記事ページのみ**：記事編集画面で canonical URL が指定されているときだけ 1（展開）になります。`<link rel="canonical">` を `<head>` に出すために使います。`{entry_canonical_url}` を中で参照します。 |
| 記事関連 | `entry_noindex` | **個別記事ページのみ**：記事編集画面で「検索エンジンに表示しない」をオンにした記事でだけ 1（展開）になります。`<meta name="robots" content="noindex,follow">` を出すために使います。 |
...
| `{entry_excerpt}` | 記事の要約（meta description / OG 用）。記事編集画面の「要約」が入力されていればその値、空なら本文を 200 字でクリップ（タグ除去）、本文も空ならタイトル。一覧・個別記事ページのどちらでも展開されます。SB3 の `sum` 互換 |
