...
テンプレートのセットは複数保存でき、「デザイン」で利用するテンプレートを切り替えられます。また、アーカイブ表示やプロフィール表示に適用するテンプレートを個別に設定することもできます。カテゴリーごとにテンプレートを変えることも可能です。
...
| 記事関連 | `pinned_entry` | **個別記事ページのみ**：ピン留めされている記事では 1、そうでなければ 0 です。一覧ページ（トップ・カテゴリーなど）では常に 0 になります（一覧ページでのピン状態には `{entry_pinned}` タグを使用してください）。 |
| 記事関連 | `option` | 個別記事表示モードのときにのみ表示されます。 |
| 記事関連 | `sequel` | 個別記事表示モードのときにのみ表示されます。前後の記事へのナビゲーションを含みます。 |
| 記事関連 | `entry_canonical` | **個別記事ページ・固定ページのみ**：編集画面で canonical URL が指定されているときだけ 1（展開）になります。`<link rel="canonical">` を `<head>` に出すために使います。`{entry_canonical_url}` を中で参照します。 |
| 記事関連 | `entry_noindex` | **個別記事ページ・固定ページのみ**：編集画面で「検索エンジンに表示しない」をオンにしたときだけ 1（展開）になります。`<meta name="robots" content="noindex,follow">` を出すために使います。 |
| 記事関連 | `single_meta` | **個別記事ページ・固定ページのみ**：`<head>` の meta description / OG タグなど、単一コンテンツ表示でのみ意味を持つメタ情報をまとめるブロックです（`{entry_excerpt}` 等を中で参照）。一覧ページでは `{entry_excerpt}` が先頭記事の要約に解決してしまうため展開されません。 |
...
| リスト関連 | `selected_entry` | 現在表示中の記事（検索結果・アーカイブ・カテゴリーの記事一覧、または個別記事ページではその記事と前後記事）を `{selected_entry_list}` にリスト表示します。表示対象がないときは 0（非表示）になります。 |
| 固定ページ | `dedicated_page` | 固定ページでのみ表示されるブロックです。通常の記事ページや一覧ページでは表示されません。 |

### ピン留め記事の表示（`{entry_pinned}` タグと `pinned_entry` ブロック）

記事にピン留めフラグを設定すると、トップページおよびカテゴリーアーカイブの 1 ページ目の先頭に浮上させることができます。テンプレートでピン留め状態に応じた見た目を変えるには、次の方法を使います。

**一覧ページ（トップ / カテゴリー）** では `{entry_pinned}` タグを使います。ピン留め記事は `"pinned"`、通常記事は `""` になるため、CSS クラスとして直接注入できます。
...
**個別記事ページ**では `pinned_entry` ブロックも使えます（count = 1 / 0 でピン状態を制御）。

```html
<!-- BEGIN entry -->
<!-- BEGIN pinned_entry -->
<span class="pin-badge">📌 ピン留め記事</span>
<!-- END pinned_entry -->
<h2>{entry_title}</h2>
<!-- END entry -->
```

> **注意:** `pinned_entry` ブロックは一覧ページでは常に 0（非表示）です。一覧ページでのピン状態の分岐には `{entry_pinned}` タグを使ってください。
...
| `{custom_xxx}` | ユーザー登録のカスタムタグ。管理画面で登録した名前と値が展開されます。 |
...
| `{entry_title}` | 記事タイトル |
| `{entry_date}` | 記事の日付。一覧ページ・個別記事ページのどちらでも「デザイン > 設定 > 時刻表記設定 > 記事日付」（既定: `%Year%-%Mon%-%Day% (%Week%)`）で整形されます。 |
...
| `{entry_excerpt}` | 記事の要約（meta description / OG 用）。編集画面の「要約」が入力されていればその値、空なら本文を 200 字でクリップ（タグ除去）、本文も空ならタイトル。一覧・個別記事ページ・固定ページのいずれでも展開されます。SB3 の `sum` 互換 |
| `{entry_tags}` | タグ一覧の HTML フラグメント |
| `{entry_pinned}` | ピン留めされている記事では `"pinned"`、そうでなければ `""` （空文字）。CSS クラスとして直接使えます（例：`class="entry {entry_pinned}"`）。一覧ページ・個別記事ページともに per-entry で正しく動作します。 |
...
| `{entry_canonical_url}` | canonical URL。`entry_canonical` ブロック内でのみ意味を持ちます（URL 未指定の記事では `entry_canonical` 自体が展開されません） |
...
| タグ | 内容 |
|---|---|
| `{recent_comment_list}` | 最近のコメントリストの `<ul><li>EntryTitle<br />=&gt; <a href="...">NameDate</a></li>...</ul>` HTML フラグメント（SB3 `_comment` 互換）。`Date` は「デザイン > 設定 > 時刻表記設定 > リスト」の書式（既定: ` (%Mon%/%Day%)`）。 |

### `latest_entry` ブロック内のタグ

| タグ | 内容 |
|---|---|
| `{latest_entry_list}` | 最新記事リストの `<ul><li><a href="...">Title</a>Date</li>...</ul>` HTML フラグメント（SB3 `_latest` 互換）。`Date` は「デザイン > 設定 > 時刻表記設定 > リスト」の書式（既定: ` (%Mon%/%Day%)`）。 |
...
テンプレート 1 セットの編集はこのページで扱いますが、「どのテンプレートを使うか」「サイト共通の表示設定」「カスタムタグ」「インポート / エクスポート」「OG カードの既定値」は [ブログデザインと OG カード](design) にまとめています。
