...
下表は主なライブラリの抜粋（`go.mod` の直接依存）です。これらを含め、**バイナリにコンパイルされる全モジュール（間接依存を含む）** の著作権表示とライセンス全文を、配布物の `THIRD-PARTY-NOTICES.txt` に収録しています。同ファイルは `go list -deps ./cmd/serenebach` の import graph に出るモジュールから生成しています。
...
バイナリに含まれる全モジュールのライセンス全文は `THIRD-PARTY-NOTICES.txt` を、依存バージョンの一覧は `go.mod` / `go.sum` を参照してください。
