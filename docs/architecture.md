...
- **Settings** — pin a template per route family. Archive (year / month / category / tag) pages use the archive pin when set; everything else stays on the active template. Profile pin is stored but the profile route is deferred to a later phase.
